...
export function CompanyShowcase() {
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-display font-bold text-primary">ORGANIZATIONS</h3>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {COMPANIES.map((company, index) => (
          <motion.div
            ...
            className="steam-panel border border-border/30 p-4 flex flex-col items-center justify-center gap-3 hover:border-primary/60 transition-all group cursor-pointer"
          >
            <img 
              src={company.logo} 
              alt={company.name}
            ...
            <div className="text-center">
              <p className="font-mono font-bold text-xs text-foreground group-hover:text-primary transition-colors">{company.name}</p>
              <p className="text-[10px] text-muted-foreground mt-1">{company.role}</p>
            </div>
          </motion.div>
        ))}
      </div>
